    ...
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            if line.startswith('-r') or line.startswith('--requirement'):
                ...
                data = _read_requirements_file(filename, data)
                continue
...
class RequirementsParser(object):
    ...
    def __init__(self, path='', name='requirements', extn='txt'):
        ...
        paths += glob(_build_filename(path, '%s[-_]*.%s', name, extn))

        for f in paths:
            source_slice = slice(len(name) + 1, -(len(extn) + 1))
            source = os.path.basename(f)[source_slice] or '*'
            self.data[source] = _read_requirements_file(f)

        # Handle dependency links file if available:
        path = _build_filename(path, '%s.%s', 'dependency_links', extn)
